...
import { Calendar, MapPin, Clock, Info, Tag } from 'lucide-react';
...
function EventDetails() {
  ...
  const handleResalePurchase = async (ticketId: string, price: string) => {
    ...
    try {
      setIsLoading(true);
      const toastId = toast.loading(`Processing purchase of ticket ${ticketId}...`);
      
      await contractService.purchaseResaleTicket(
        Number(id),
        Number(ticketId),
        resaleTickets.find(t => t.id === ticketId)?.seller || '',
        price
      );

      toast.success('Resale ticket purchased successfully!', { id: toastId });
    } catch (error) {
                ...
                <button
                  ...
                  className={`w-full ${
                    isLoading || !isInitialized
                      ? 'bg-blue-500 cursor-not-allowed'
                      : 'bg-blue-600 hover:bg-blue-700'
                  } text-white font-medium py-2 sm:py-3 px-4 rounded-lg transition-colors text-sm sm:text-base`}
                >
                  {!account ? 'Connect Wallet to Purchase' : isLoading ? 'Processing...' : 'Purchase Tickets'}
                </button>
              </div>
