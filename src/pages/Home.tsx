...
function Home() {
  ...
  const handleChatbotSubmit = (e: React.FormEvent) => {
    ...
    if (!event) {
      setRecommendation({ error: `Event with ID ${eventId} not found.` });
      return;
    }

    const analysis = analysisService.analyzeEvent(event);
    ...
    <div className="space-y-6 md:space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:space-x-4 w-full sm:w-auto">
          ...
          <select 
            className="w-full sm:w-auto bg-gray-800 text-white px-4 py-2 rounded-lg border border-gray-700 text-sm"
            value={sortBy}
              ...
              <div className="mt-4 space-y-3 bg-gray-700/50 rounded-lg p-4">
                <div>
                  ...
                  <span className={`ml-2 font-semibold ${
                    recommendation.recommendation === 'Strong Buy' ? 'text-green-400' :
                    recommendation.recommendation === 'Buy' ? 'text-green-300' :
                    recommendation.recommendation === 'Moderate Buy' ? 'text-blue-400' :
                    recommendation.recommendation === 'Hold' ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>{recommendation.recommendation}</span>
                </div>
                <div>
                  <span className="text-gray-300">Expected ROI:</span>
                  <span className="ml-2 text-white">{recommendation.expected_roi}%</span>
                </div>
                <div>
                  <span className="text-gray-300">Risk Level:</span>
                  <span className={`ml-2 ${
                    recommendation.risk_level === 'Low' ? 'text-green-400' :
                    recommendation.risk_level === 'Medium' ? 'text-yellow-400' :
                    'text-red-400'
                  }`}>{recommendation.risk_level}</span>
                </div>
                <div>
                  <span className="text-gray-300">Suggested Holding:</span>
